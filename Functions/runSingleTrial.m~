function trialData  = runSingleTrial(trial, design, visual, settings, t)
...
%     fixColor = visual.fixColor;
    ...
    Eyelink('Message', 'TRIAL SYNCTIME');
    ...
    Eyelink('Message', 'STIM ON SCREEN');
    ...
    while on_fix_hand && isnan(t_cross)
        ...
        while isnan(t_go) % set time stamp the first time this is executed
            Eyelink('Message', 'ATTACKER MOVED');
        ...
        elseif isnan(t_disap)
            Eyelink('Message', 'ATTACKER DISAPPEARED');
...
%             dataLog.message = [dataLog.message, sprintf('The hand moved at %f\n', t_movStartPixx)];f
        ...
        if status.newLogFrames                                              % something new happened
            ...
            if ~ goresp && ...
               ...
               Eyelink('Message', 'END HAND MOVEMENT');
        ...
        if attackerPos(1) >= goalPos(1)
            Datapixx('RegWrRd');
            Eyelink('Message', 'ATTACKER REACHED GOAL');
        ...
        % check if eyetracker is still recording:
        error = Eyelink('EyeAvailable');
        if error == -1
...
%     dataLog.message = [dataLog.message, sprintf('Trial End at %f \n', t_end)];
    ...
    Eyelink('Message', 'FEEDBACK PRESENTED');
    
    Screen('Flip', visual.window);
    
    Eyelink('Message', 'RESPONSE TRIAL %i, %s', t, response);
